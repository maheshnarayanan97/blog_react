...
const Blog = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get('https://forcode-kerala-tourist-app.herokuapp.com/api/blog/featured');
    ...
    useEffect(() => {
        const fetchBlogs = async () => {
            try {
                const res = await axios.get('https://forcode-kerala-tourist-app.herokuapp.com/api/blog/');
                setBlogs(res.data);
                console.log(res.data);
            }
            catch (err) {
            ...
            return list.push(
                <div className="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                    <div className="col p-4 d-flex flex-column position-static">
                        ...
                        <div className="mb-1 text-muted">{blogPost.month} <span>{blogPost.day}</span> </div>
                        <p className="card-text mb-auto">{blogPost.excerpt}</p>
                        <Link to={`/blog/${blogPost.slug}`} className="stretched-link">Continue reading</Link>
                    </div>
                    <div className="col-auto d-none d-lg-block">
                        <img width='200' height='250' src={blogPost.thumbnail_image} alt='thumbnail' />
                    </div>
                </div>
            );
        });
